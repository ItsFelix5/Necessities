...
public class ModBlocks {
    ...
    public static final Block TINY_POTATO = register(
            ...
            s->s.food(new FoodComponent.Builder().alwaysEdible().build(), ConsumableComponents.food()
                    .consumeEffect(new ApplyEffectsConsumeEffect(new StatusEffectInstance(StatusEffects.NAUSEA, 500, 1), 1.0f))
                            .build())
    );
    ...
    private static Block register(String name, Function<AbstractBlock.Settings, Block> factory, Function<AbstractBlock.Settings, AbstractBlock.Settings> blockSettings,
        ...
        Block block = factory.apply(blockSettings.apply(AbstractBlock.Settings.create()).registryKey(blockKey));

        RegistryKey<Item> itemKey = RegistryKey.of(RegistryKeys.ITEM, Main.id(name));
        BlockItem blockItem = new BlockItem(block, itemSettings.apply(new Item.Settings()).registryKey(itemKey));
