...
public class LashingPotatoHookEntityRenderer extends EntityRenderer<LashingPotatoHookEntity> {
    ...
    @Override
    public Identifier getTexture(LashingPotatoHookEntity entity) {
        return null;
    }

    @Override
    public void render(LashingPotatoHookEntity entity, float yaw, float tickDelta, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int light) {
        matrixStack.push();
        ...
        if (this.dispatcher.gameOptions.getPerspective().isFirstPerson() && holder == MinecraftClient.getInstance().player) {
            ...
            vec3d3 = holder.getCameraPosVec(tickDelta).add(vec3d);
        } else {
            ...
            vec3d3 = holder.getCameraPosVec(tickDelta).add(-e * j - d * k, (double) l - 0.45 * (double) h, -d * j + e * k);
        }

        float j = tickDelta * 0.15F;
        vec3d3 = vec3d3.subtract(MathHelper.lerp(tickDelta, entity.prevX, entity.getX()),
                MathHelper.lerp(tickDelta, entity.prevY, entity.getY()) + entity.getStandingEyeHeight(),
    ...
    @Override
    public boolean shouldRender(LashingPotatoHookEntity entity, Frustum frustum, double x, double y, double z) {
        return true;
    }
}
